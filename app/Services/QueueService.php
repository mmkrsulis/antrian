...
{
    public function issue(int $serviceId): array
    {
        ...
        try {
            ...
            $publicId = $this->uuid();
            $db->prepare('INSERT INTO tickets(public_id, service_id, queue_date, sequence_number, ticket_number) VALUES (?, ?, ?, ?, ?)')
                ->execute([$publicId, $serviceId, $date, $next, $number]);
            $id = (int) $db->lastInsertId();
            $db->prepare("INSERT INTO queue_events(ticket_id, event_type, payload) VALUES (?, 'issued', ?)")->execute([$id, json_encode(['ticket_number' => $number])]);
            $db->commit();
            Audit::log('ticket.issued', 'ticket', $id, ['ticket_number' => $number]);
