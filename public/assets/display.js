...
function speakNext(){if(speaking||!queue.length||!audioEnabled)return;speaking=true;const item=queue.shift(),generation=++announcementGeneration;number.textContent=item.ticket_number;counter.textContent=(item.counter_name||'Loket').toUpperCase();document.querySelector('.display-center').classList.add('announcing');playRecordedAnnouncement(item,generation).then(()=>{if(generation===announcementGeneration)setTimeout(()=>{if(generation===announcementGeneration)finishAnnouncement()},1800)}).catch(error=>{if(generation!==announcementGeneration||error.message==='cancelled')return;if(error.name==='NotAllowedError'){audioEnabled=false;speaking=false;queue.unshift(item);document.querySelector('.display-center').classList.remove('announcing');audioButton.hidden=false;return}browserSpeechFallback(item)})}
