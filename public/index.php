...
try {
    ...
    if ($path === '/api/chat/messages' && $method === 'GET') {
        ...
        $recentStmt=$db->prepare('SELECT id,user_id,recipient_user_id,sender_name,body,created_at FROM chat_messages WHERE recipient_user_id IS NULL OR user_id=? OR recipient_user_id=? ORDER BY id DESC LIMIT 1000');$recentStmt->execute([$user['id'],$user['id']]);$latest=[];foreach($recentStmt->fetchAll() as $item){$key=$item['recipient_user_id']===null?0:((int)$item['user_id']===(int)$user['id']?(int)$item['recipient_user_id']:(int)$item['user_id']);if(!isset($latest[$key]))$latest[$key]=$item;}
        $groupMessage=$latest[0]??null;$conversations=[['id'=>0,'name'=>'Grup Semua','role'=>'group','online'=>true,'last_message'=>$groupMessage['body']??'Belum ada pesan di grup','last_message_at'=>$groupMessage['created_at']??null,'last_sender_name'=>$groupMessage['sender_name']??null,'unread'=>(int)($unreadBy[0]??0)]];foreach($users as $chatUser){$id=(int)$chatUser['id'];$last=$latest[$id]??null;$conversations[]=['id'=>$id,'name'=>$chatUser['name'],'role'=>$chatUser['role'],'online'=>(bool)$chatUser['online'],'last_message'=>$last['body']??'Belum ada pesan','last_message_at'=>$last['created_at']??null,'last_sender_name'=>$last['sender_name']??null,'unread'=>(int)($unreadBy[$id]??0)];}$group=array_shift($conversations);usort($conversations,static fn($a,$b)=>strcmp((string)($b['last_message_at']??''),(string)($a['last_message_at']??''))?:strcmp((string)$a['name'],(string)$b['name']));array_unshift($conversations,$group);
        $json(['messages'=>$messages,'users'=>$users,'conversations'=>$conversations,'peer'=>$peer,'unread'=>$unread,'unread_by_conversation'=>$unreadBy]);
    }
