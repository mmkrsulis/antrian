...
[ -n "$token" ]
dashboard="$(curl -fsS -L -b "$cookie_file" -c "$cookie_file" --data-urlencode "_csrf=$token" --data-urlencode 'username=admin' --data-urlencode 'password=AdminLive123!' "$base_url/login")"
